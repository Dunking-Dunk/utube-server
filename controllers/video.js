...
export const updateVideo = async (req, res, next) => {
  try {
    ...
    if (!video) return next(new createError(404, "Video not found"));
...
export const getUsersVideo = async (req, res, next) => {
  try {
    const videos = await Video.find({ userId: req.user.id }).limit(40);
    res.status(200).json(videos);
  } catch (err) {
    next(err);
  }
